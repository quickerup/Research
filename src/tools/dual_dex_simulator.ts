...
export interface SimulatorOptions {
  ...
  slippage: number; // e.g. 0.005
  mock: boolean;
}

export interface MatchedQuoteResult {
  amountTon: number;
  stonfiQuoteUsdt: number;
  dedustQuoteUsdt: number;
  pathA: {
    description: string;
    leg1DeDustTonToUsdt: number;
    leg2StonfiUsdtToTon: number;
    grossProfitTon: number;
    grossRoiPct: number;
    netProfitTon: number;
    netRoiPct: number;
  };
  pathB: {
    description: string;
    leg1StonfiTonToUsdt: number;
    leg2DeDustUsdtToTon: number;
    grossProfitTon: number;
    grossRoiPct: number;
    netProfitTon: number;
    netRoiPct: number;
...
export const MOCK_SIMULATOR_DATA: Record<number, { stonfiUsdt: number; dedustUsdt: number; stonfiTonBack: number; dedustTonBack: number }> = {
  1: {
    stonfiUsdt: 1.377746,
    dedustUsdt: 1.575724,
...
export async function simulateDualDexArbitrage(options: SimulatorOptions): Promise<MatchedQuoteResult[]> {
  const results: MatchedQuoteResult[] = [];
  ...
  for (const amountTon of options.amounts) {
    ...
    if (options.mock) {
      const mockEntry = MOCK_SIMULATOR_DATA[amountTon] || {
      ...
      try {
        ...
        const mockEntry = MOCK_SIMULATOR_DATA[amountTon] || MOCK_SIMULATOR_DATA[10];
        leg1DeDustUsdt = mockEntry.dedustUsdt;
        leg2StonfiTon = mockEntry.stonfiTonBack;
        leg1StonfiUsdt = mockEntry.stonfiUsdt;
        leg2DeDustTon = mockEntry.dedustTonBack;
