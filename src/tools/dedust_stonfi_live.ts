...
export interface DedustPoolReserves {
...
export async function fetchDedustPoolReserves(poolAddress: string): Promise<DedustPoolReserves> {
  const response = await axios.post(`${DEDUST_V4_BASE}/get_pools`, {
    offset: 0,
    limit: 1,
    sort_by: 'volume_24h',
    sort_direction: 'desc',
...
export function constantProductSwapOut(amountIn: bigint, reserveIn: bigint, reserveOut: bigint, feeBps: number): bigint {
...
let cachedPtonMasterAddress: string | null = null;

export async function getPtonMasterAddress(): Promise<string> {
  if (cachedPtonMasterAddress) return cachedPtonMasterAddress;
  const response = await axios.get('https://api.ston.fi/v1/routers', { timeout: 5000 });
  ...
  const best = routers.reduce((a: any, b: any) => (b.major_version > a.major_version ? b : a));
  cachedPtonMasterAddress = best.pton_master_address;
  return cachedPtonMasterAddress as string;
}
