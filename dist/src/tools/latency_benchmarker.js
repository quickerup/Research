...
async function benchmarkLatency(options) {
    const stonfiLatencies = [];
    const dedustLatencies = [];
    if (options.mock) {
        stonfiLatencies.push(...exports.MOCK_LATENCIES.stonfi.slice(0, options.samples));
        ...
        for (let i = 0; i < options.samples; i++) {
            ...
            try {
                ...
                await axios_1.default.post(stonfiUrl, {}, { headers, timeout: 5000 });
            ...
            try {
                const t0 = perf_hooks_1.performance.now();
                await axios_1.default.post(dedustUrl, dedustPayload, { headers: { ...headers, 'Content-Type': 'application/json' }, timeout: 5000 });
                const t1 = perf_hooks_1.performance.now();
