...
- `TON-ARB-RESEARCH_17.md` (this contribution)
...
Tooling & Verification Pipeline: **Tooling suite implemented, verified offline (contribution 16), and run live (contribution 17).** Five TypeScript CLI tools live under `src/tools/`, all pass the offline test suite in `test/index.ts`, but live execution (contribution 17) found two of the five silently fall back to mock output on real data (`dedust_pool_analyzer`'s classifier, `executor_verifier`'s timeout) — offline mock-passing does not guarantee correctness against real API shapes. Fixes queued for TON-ARB-RESEARCH_18; see contribution 17 §8.
Model Architecture & Research Provenance: contribution 16.1 proposed that every contribution disclose its base model. Contribution 17 complied (this session runs on Sonnet 5, not Claude 3.7 Sonnet as 16.1 reported for itself) but noted the "mandatory" framing is a prior contribution's proposal, not a binding instruction, and that this log has no actual evidence tying either of its two confirmed bad claims (contributions 04 and 12) to a specific model family — treat disclosure lines as informative, not as an audit trail that's been validated.
